...
public class MultiPlayerGUI extends JPanel {
	...
	public MultiPlayerGUI(User u, String selectedSong, String socket) {
		s = socket;
		songs = selectedSong;
	...
	public void initializeVariables() {
		try {
		      ...
		      font = ttfBase.deriveFont(Font.BOLD, 26);
	...
	public void updateOScore(int s)
	{
		opponentscore = s;
		updateLabels();
	}
	...
	public void hit(String direction, boolean perfect) {
		...
		/*
		 * update server with score. (send the score the other user every time they hit, aka every time the score goes up)
		 */
		
		client.Client.csc.sendMessage("UPDATE_OPP_SCORE/" + s + "/" + score + "/");
	}
	
	/*
	 * create a method like updateScore(int score) that updates the variable opponentscore, then call updateLabels()
	 */
	...
	public void endGame() {
		...
		Client.csc.updateScore(user.getName(), score);
		Client.csc.addGame(user.getName());
		Client.csc.updateLast(songs);
		user.setLastSong(songs);
